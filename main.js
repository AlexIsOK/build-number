...
function main() {
    ...
    const currentVersion = `${env.INPUT_VERSION}`;
    //${currentVersion}
    ...
    request('GET', `/repos/${env.GITHUB_REPOSITORY}/git/refs/tags/${prefix}${currentVersion}-`, null, (err, status, result) => {
        ...
        if (status === 404) {
            ...
            const regexString = `/${prefix}${currentVersion}-(\\d+)$`;
            const regex = new RegExp(regexString);
            nrTags = result.filter(d => d.ref.match(regex));
            
            const MAX_OLD_NUMBERS = 10000; //One or two ref deletes might fail, but if we have lots then there's something wrong!
            if (nrTags.length > MAX_OLD_NUMBERS) {
                fail(`ERROR: Too many ${prefix}${currentVersion}- refs in repository, found ${nrTags.length}, expected only 1. Check your tags!`);
            }
        ...
        let newRefData = {
            ref:`refs/tags/${prefix}${currentVersion}-${nextBuildNumber}`, 
            sha: env.GITHUB_SHA
        };
